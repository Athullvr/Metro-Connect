...
export default function App() {
  ...
  const handleSelectFromMap = (stationName, target) => {
    // If on itinerary or disruption, go to home
    setScreen('home');
  };

  return (
    <div className="min-h-screen bg-[#F8FAFC] text-slate-900 flex flex-col font-sans select-none antialiased relative">
      
      {/* Official KMRL Transit Header */}
      <header className="transit-header sticky top-0 z-40 px-3.5 sm:px-8 py-3 flex items-center justify-between border-b border-slate-200/80 shadow-xs">
        <button
          type="button"
          className="flex items-center gap-2.5 sm:gap-3.5 cursor-pointer text-left group transition-transform duration-200 hover:scale-[1.01]"
          onClick={() => setScreen('home')}
          aria-label="Go to journey planner"
        >
          {/* KMRL Chevron Emblem Logo */}
          <div className="w-8 h-8 sm:w-9 sm:h-9 rounded-xl bg-gradient-to-br from-[#009999] to-[#007A7A] flex items-center justify-center text-white font-bold text-xs tracking-tight font-display shadow-md shadow-teal-700/20 group-hover:shadow-teal-600/30 transition-all duration-300 shrink-0">
            <svg viewBox="-258 191.7 441.7 178.3" className="w-5 h-5 sm:w-6 sm:h-6 fill-white" aria-hidden="true">
          ...
          <div>
            <div className="flex items-center gap-1.5 sm:gap-2">
              <span className="text-sm sm:text-base font-extrabold text-slate-900 tracking-tight font-display">
                KOCHI METRO
              </span>
              <span className="text-[9px] sm:text-[10px] font-bold uppercase tracking-wider px-1.5 sm:px-2 py-0.5 rounded-full bg-[#e6f8f7] text-[#009999] border border-[#99dedb]">
                CONNECT
              </span>
            </div>
            <p className="text-[10px] sm:text-[11px] text-slate-500 font-medium hidden xs:block">Rail • Water Metro • Feeder</p>
          </div>
        </button>
        ...
        {/* Engine Status Badge */}
        <div className="flex items-center gap-1.5 sm:gap-2 bg-white border border-slate-200/90 px-2.5 sm:px-3 py-1 sm:py-1.5 rounded-full text-xs shadow-2xs">
          <span
            className={`w-2 h-2 rounded-full shrink-0 ${useSimulator ? 'bg-amber-500 animate-pulse' : 'bg-[#22c55e]'}`}
            aria-hidden="true"
          />
          <span className="text-[10px] sm:text-[11px] font-semibold text-slate-700">
            {useSimulator ? 'Offline Graph' : 'Live AI'}
          </span>
        </div>
      </header>

      {/* Main Content Viewport with mobile bottom bar spacing */}
      <main className="flex-grow flex flex-col items-center justify-start relative z-10 w-full pb-24 sm:pb-8">
        {screen === 'home' && (
          <Home
      ...
      {/* Mobile Sticky Bottom Navigation Bar (Dock) */}
      <nav className="sm:hidden fixed bottom-0 left-0 right-0 z-50 bg-white/95 backdrop-blur-lg border-t border-slate-200/90 px-4 py-2 flex items-center justify-around shadow-lg safe-bottom">
        <button
          type="button"
          onClick={() => setScreen('home')}
          className={`flex flex-col items-center gap-1 py-1 px-3 rounded-xl transition-all duration-200 cursor-pointer ${
            screen === 'home' || screen === 'itinerary'
              ? 'text-[#009999] font-bold scale-105'
              : 'text-slate-500 hover:text-slate-900'
          }`}
        >
          <Navigation size={18} className={screen === 'home' || screen === 'itinerary' ? 'text-[#009999]' : 'text-slate-400'} />
          <span className="text-[10px] font-display">Planner</span>
        </button>

        <button
          type="button"
          onClick={() => setShowStationsModal(true)}
          className="flex flex-col items-center gap-1 py-1 px-3 rounded-xl text-slate-500 hover:text-slate-900 transition-all duration-200 cursor-pointer"
        >
          <Compass size={18} className="text-slate-400" />
          <span className="text-[10px] font-display">Stations</span>
        </button>
        ...
        <button
          ...
          className={`flex flex-col items-center gap-1 py-1 px-3 rounded-xl transition-all duration-200 cursor-pointer ${
            screen === 'disruption'
              ? 'text-[#dc2626] font-bold scale-105'
              : 'text-slate-500 hover:text-slate-900'
          }`}
        >
          <AlertTriangle size={18} className={screen === 'disruption' ? 'text-[#dc2626]' : 'text-slate-400'} />
          <span className="text-[10px] font-display">Alerts</span>
        </button>
      </nav>

      {/* Global Stations Modal (Accessible via Mobile Dock) */}
      <NetworkMapModal
        isOpen={showStationsModal}
        onClose={() => setShowStationsModal(false)}
        onSelectStation={(st, target) => {
          setShowStationsModal(false);
          setScreen('home');
        }}
      />

      {/* Bottom Footer (Desktop / Tablet Branding) */}
      <footer className="hidden sm:block py-6 px-4 text-center text-xs font-medium text-slate-500 bg-white border-t border-slate-200">
